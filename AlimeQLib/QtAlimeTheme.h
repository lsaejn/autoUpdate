...
class QtAlimeTheme : public QObject
{
    Q_OBJECT

public:
    explicit QtAlimeTheme(QObject *parent = 0);
    ~QtAlimeTheme();
...
protected:
    const QScopedPointer<QtAlimeThemePrivate> d_ptr;

private:
    Q_DISABLE_COPY(QtAlimeTheme)
    Q_DECLARE_PRIVATE(QtAlimeTheme)
};
